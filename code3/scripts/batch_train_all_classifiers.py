...
DATA_ROOT = ROOT / "results" / "face_parts"
...
def main():
    ...
    parts = {
        'face': (DATA_ROOT / 'face', 3),
        'mouth': (DATA_ROOT / 'mouth', 3),
        'nose': (DATA_ROOT / 'nose', 3),
        'eyes': (DATA_ROOT / 'eyes', 3),
        'ear': (DATA_ROOT / 'ear', 2)  # 只有2类
    }
    ...
    # 保存为JSON
    with open(RESULTS_DIR / 'all_classifiers_results.json', 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=4, ensure_ascii=False)
    
    # 保存为CSV  
    df = pd.DataFrame(results)
    df.to_csv(RESULTS_DIR / 'all_classifiers_results.csv', index=False, encoding='utf-8-sig')
    
    # 生成总结报告
    generate_summary_report(results, RESULTS_DIR / 'classifier_summary.txt')
    
    print("\n✓ 结果已保存:")
    print(f"  - {RESULTS_DIR / 'all_classifiers_results.json'}")
    print(f"  - {RESULTS_DIR / 'all_classifiers_results.csv'}")
    print(f"  - {RESULTS_DIR / 'classifier_summary.txt'}")
    print()


def generate_summary_report(results, output_path: Path):
    """生成总结报告"""
